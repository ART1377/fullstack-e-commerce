...
const DashboardEditProductForm = ({ colors, product }: Props) => {
  ...
  const [cloudinaryImages, setCloudinaryImages] = useState<string[]>([]); // New state for previews
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.state.success) {
        ...
        setFormState(result);
        toast.error("ویرایش محصول ناموفق بود. لطفا دوباره تلاش کنید.");
      }
    } catch (error) {
  ...
  useEffect(() => {
    const receivedFeatures = product.features?.map((feature) => {
      return {
        id: feature.id,
        title: feature.title,
    ...
    const receivedStock = product.stock?.map((stockItem) => {
      return {
        id: stockItem.id,
        color: stockItem.colorId!,
        quantity: stockItem.quantity,
        size: stockItem.size,
      };
    });
    const receivedImages = product.images?.map((image) => image.url);

    setCloudinaryImages(receivedImages!);
    setStockItems(receivedStock!);
    setFeatures(receivedFeatures!);
  }, [product]);
  ...
  const handleRemoveOldImage = (imageUrl: string) => {
    const images = cloudinaryImages.filter((url) => url !== imageUrl);
      ...
      <DashedContainer label="تصاویر">
        <div className="flex flex-col gap-4 w-full">
          ...
          {imagePreviews.length > 0 && (
            <div className="flex flex-col gap-4">
              <p className="text-dark text-bodyMain">تصاویر افزوده شده : </p>
          ...
          {cloudinaryImages?.length! > 0 && (
            <div className="flex flex-col gap-4 pt-4 border-t border-customGray-300">
              <p className="text-dark text-bodyMain">تصاویر موجود : </p>
              <div className="flex flex-wrap gap-4">
...
export default DashboardEditProductForm;
